...
contract('PoA & TST', ([, propertySeller, buyer, anotherBuyer]) => {
  ...
  beforeEach(async () => {
    this.poa = await PoAToken.new(
      propertySeller,
      totalSupply,
      tokensForEther,
      metaData
    )
    this.tst = await TimeShareToken.at(
      await this.poa.getTimeShareTokenAddress()
    )
  })
  ...
  it('is not possible to buy more tokens than available', async () => {
    assertRevert(
      this.poa.sendTransaction({ value: web3.toWei(11, 'ether'), from: buyer })
    )
  })
  ...
  it("can't send ether to TST", async () => {
    assertRevert(
      this.tst.sendTransaction({ value: web3.toWei(1, 'ether'), from: buyer })
    )
  })
  ...
  describe('When buyer holds tokens for some time', () => {
    ...
    it('emits UpdateTimeShare events', async () => {
      const { logs } = await this.poa.transfer(anotherBuyer, toToken(5), {
        from: buyer
      })
      ...
      logs[1].args.balance.should.be.bignumber.equal(
        timeShareBalanceAfter5Blocks / 2
      )
    ...
    it('can approve and transferFrom token', async () => {
      ...
      await this.poa.transferFrom(buyer, anotherBuyer, toToken(5), {
        from: anotherBuyer
      })
    ...
    it('emits ClaimTimeShareTokens event', async () => {
      const { logs } = await this.poa.claimTimeShareTokens(
        timeShareBalanceAfter5Blocks / 2,
        { from: buyer }
      )
      ...
      logs[2].args.amount.should.be.bignumber.equal(
        timeShareBalanceAfter5Blocks / 2
      )
    })
    ...
    it("can't claim TST above available amount", async () => {
      assertRevert(
        this.poa.claimTimeShareTokens(timeShareBalanceAfter5Blocks + 1, {
          from: buyer
        })
      )
    })
    ...
    describe('When holder has 1 TST', () => {
      ...
      it("can't open the door if not booked", async () => {
        ...
        const signature = await web3.eth.sign(
          buyer,
          soliditySha3(String(timestamp))
        )

        const canOpenDoor = await this.tst.isValidAccessKey(
          timestamp,
          signature
        )
